...
class StaffContactsBlock
{
    private static function processData($query)
    {
        $arrayPropsCodes = ['STAFF_CONTACTS'];
        ...
        foreach ($result as &$block) {
            ...
            $block['ITEMS'] = StaffContacts::getByIds(['ids' => $block['STAFF_CONTACTS']]);
            $block['LINK'] = Links::getById(['id' => $block['LINK']]);
            $block['FORM'] = ButtonForms::getById(['id' => $block['FORM']]);
            unset($block['STAFF_CONTACTS']);
        }
    ...
    public static function getByIds($arRequest)
    {
        ...
        if (Loader::includeModule('iblock')) {
            $q = IblockPropertiesTable::query()
                ->withIblock(Constants::IB_STAFF_CONTACTS_BLOCK)
                ->withSelect()
                ->withFilterByIDs($ids);
